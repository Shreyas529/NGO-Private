...
def user_ui(db):
    ...
    if 'selected_ngo' in st.session_state:
        display_selected_ngo(ngo_db, st.session_state['selected_ngo'])
    else:
...
def donate_funds(ngo_db):
    ...
    for ngo in ngos:
        if ngo['Name'] == selected_ngo:
            selected_ngo_metamask=ngo.get('metamask_address', 'No metamask address available')
    col=st.columns(2)
    with col[1]:
        st.markdown(f'''##
                             {selected_ngo_metamask}''')
    with col[0]:
        st.markdown("*Metamask Public Key:*")
